...
const TurnIndicator = ({ isMyTurn, playerName, className = '', variant = 'cards', ...props }) => {
    const style = {
        padding: variant === 'bingo' ? '6px 20px' : '10px 30px',
        borderRadius: variant === 'bingo' ? '30px' : '50px',
        backgroundColor: isMyTurn ? 'var(--success-color)' : 'rgba(0, 0, 0, 0.7)',
        color: 'white',
        fontWeight: 'bold',
        fontSize: variant === 'bingo' ? '1rem' : '1.2rem',
